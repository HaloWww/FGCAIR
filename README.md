...
| `auto_bind_captured` | 是否尝试自动绑定已知网关。通常保持默认值即可。 | `true` |
...
集成创建的是标准 Home Assistant `climate` 实体。要桥接到 HomeKit，请在 HomeKit Bridge 集成中选择对应的 `climate` 实体。
...
| `auto_bind_captured` | 配置向导 | 是否尝试自动绑定网关。 |
