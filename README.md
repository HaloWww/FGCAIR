...
- 使用 MQTT TLS 同步设备状态，模式和风速可通过设备主动广播实时更新。
- 主动状态查询用于刷新开关、目标温度和房间温度。
- 控制命令按单个属性逐条下发，降低设备拒绝或丢命令概率。
- 可通过 HomeKit Bridge 桥接到 Apple Home，除湿模式在 HomeKit 中显示为自动。
...
| 参数 | 默认值 | 范围 | 说明 |
| --- | --- | --- | --- |
| `update_interval` | `300` 秒 | `10` 到 `3600` 秒 | MQTT 主动状态查询间隔。模式和风速会通过设备主动广播实时更新；间隔主要影响开关和温度的兜底刷新。 |
...
| HVAC 模式 | 支持 `off`、`cool`、`heat`、`dry`，并提供 `auto` 作为 HomeKit 自动模式兼容别名。 |
...
集成通过 FGCAir 私有云 MQTT TLS 通道读取设备状态，同时监听设备主动广播的短报文。

完整状态查询会同步以下状态到 Home Assistant：
...
| 模式 | `Mode_indoor_PK4` | 从 MQTT 状态字段解析制冷、除湿、制热。 |
| 风速 | `Speed_indoor_PK4` | 从 MQTT 状态字段解析自动和 1-6 档。 |

设备主动广播的短报文会实时同步以下状态，不需要等待下一轮主动查询：

| 短报文状态 | HA 属性 | 说明 |
| --- | --- | --- |
| 模式 | `Mode_indoor_PK4` | 制冷、除湿、制热切换后实时刷新。 |
| 风速 | `Speed_indoor_PK4` | 自动和 1-6 档切换后实时刷新。 |
...
风速状态会通过 MQTT 回读。完整状态包和设备主动广播的短报文都会更新 `climate.fan_mode`。
...
| 参数 | 建议 |
| --- | --- |
| `update_interval` | 默认 `300` 秒。模式和风速通过短报文实时更新，通常不需要为了它们降低主动查询间隔。 |
| 同步字段 | 开关、目标温度、当前室温、模式和风速。 |

HomeKit 中的模式显示为 `关闭`、`制冷`、`加热`、`自动`。其中 HomeKit 的 `自动` 对应设备的 `dry` 除湿模式；从 HomeKit 或 Home Assistant `auto` 切换到自动会向设备下发除湿。
...
使用前需要显式传入自己的账号密码或 uid/token：

```bash
python tools/fgcair_cli.py --username "<your_username>" --password "<your_password>" list
```

工具产生的 `.fgcair_*.json` 缓存和 `*.jsonl` 报文记录会被 `.gitignore` 忽略，不应提交到仓库。
...
| `Mode_indoor_PK4` | 控制/状态 | 模式。MQTT 完整状态包和短报文都会更新该字段。 |
| `Speed_indoor_PK4` | 控制/状态 | 风速。MQTT 完整状态包和短报文都会更新该字段。 |
...
| HomeKit 状态不刷新 | 检查 MQTT 后台是否有状态更新；开关和温度可临时降低 `update_interval` 测试。 |
