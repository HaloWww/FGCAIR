...
配置完成后会生成每台室内机对应的 `climate` 实体、`fan` 实体和“当前室温来源”选择实体。
...
每台室内机会额外创建一个独立 `fan` 实体，用于在 Home Assistant 和 HomeKit Bridge 中控制风速。
...
因为目前无法从 MQTT 稳定读取真实风速，`fan` 实体不会用 MQTT 同步风速状态。它只保存最后一次通过 HA 或 HomeKit 下发的风速，用于展示和继续控制。

## HomeKit Bridge

集成创建标准 Home Assistant `climate` 实体和独立 `fan` 实体。要桥接到 HomeKit，请在 HomeKit Bridge 集成中选择对应的 `climate` 和 `fan` 实体。
...
HomeKit 中的风扇速度百分比会映射为 `自动` 和 `1-6档`。风速控制可以下发到设备，但真实风速状态不会通过 MQTT 回读。
